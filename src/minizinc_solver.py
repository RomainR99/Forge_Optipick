...
def allocate_with_minizinc(
    ...
    agent_type = [0 if a.type == "robot" else (1 if a.type == "human" else 2) for a in agents]
    ...
    for i, order1 in enumerate(orders):
        row = []
        products1 = [products_by_id[item.product_id] for item in order1.items if item.product_id in products_by_id]
        for j, order2 in enumerate(orders):
            if i == j:
                row.append(False)
            else:
                products2 = [products_by_id[item.product_id] for item in order2.items if item.product_id in products_by_id]
                # Vérifier si les produits sont incompatibles
                all_products = products1 + products2
                incompatible = not can_combine(all_products)
    ...
    for i, order in enumerate(orders):
        ...
        if agent_idx == 0:
            ...
            # agent_idx est 1-indexed dans MiniZinc, mais notre liste est 0-indexed
            agent_idx_python = agent_idx - 1
            if 0 <= agent_idx_python < len(agents):
                assignment[order.id] = agents[agent_idx_python].id
            else:
                assignment[order.id] = None
    ...
    for i, order in enumerate(orders):
        ...
        if agent_idx != 0:
            agent_idx_python = agent_idx - 1
            if 0 <= agent_idx_python < len(agents):
                agents[agent_idx_python].assign(order)
